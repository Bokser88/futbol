...
def get_premium_prediction(team1: str, team2: str, match_date: str) -> str:
    prompt = f"""
Ты — профессиональный каппер с 10-летним опытом, чьи прогнозы имеют высокую проходимость.
Твоя задача — дать **чёткую рекомендацию для ставок** на матч **{team1} vs {team2}**, который состоится **{match_date}**.

❗ ВАЖНО:
- Ты **не знаешь результата**, так как матч ещё не сыгран.
- Используй только гипотетическую, но правдоподобную статистику: форма команд, мотивация, очные встречи, турнирное положение.
- Пиши **уверенно, как эксперт**, без фраз «возможно», «может быть».
- Не упоминай, что ты ИИ или модель.
- Не выдумывай точные цифры, имена игроков или источники.

Дай прогноз строго по следующей структуре:

### 🔮 1. Исход матча
• Чёткая рекомендация: **П1**, **X**, **П2**  
• Вероятность (%): округли до 5%  
• Краткое обоснование (1–2 предложения)

### 📊 2. Тотал голов
• Рекомендация: **ТБ 2.5** или **ТМ 2.5**  
• Вероятность (%)  
• Обоснование

### ➕ 3. Фора
• Лучшая фора для ставки: например, **«Ф1(-1.5)»** или **«Ф2(+0.5)»**  
• Почему именно она?
...
### 🎯 5. Точный счёт
• Самый вероятный счёт (один вариант)  
• Краткое объяснение

❗ Ответ должен быть **готов к использованию в ставках** — никакой «воды», только конкретика.
"""
    try:
        r = requests.post(
